...
import com.example.healthconnect_tablet.tokens.ui.components.tokenCategoryLabel
...
private fun HistoryRow(record: PrefsStorage.ScanRecord) {
    val title = when (record.type) {
        "automated" -> tokenCategoryLabel(record.category)
        "mood" -> "Mood token"
        else -> record.type.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
    }
    val description = when (record.type) {
        "automated" -> "${record.amount} token(s) saved"
        "mood" -> record.mood ?: "Mood recorded"
        else -> "${record.amount} token(s) saved"
    }
    val icon: @Composable () -> Unit = when {
        record.type == "mood" -> {
            { MoodTokenIcon(color = TokenPalette.Green, modifier = Modifier.size(40.dp)) }
        }
        record.category == "sleep" -> {
            { SleepTokenIcon(color = TokenPalette.Blue, modifier = Modifier.size(40.dp)) }
        }
        else -> {
            { PhysicalActivityTokenIcon(color = TokenPalette.Red, modifier = Modifier.size(40.dp)) }
        }
    }
